...
class FileSystemRunRepository:
    ...
    def _load_context_from(self, run_id: str, path: Path) -> RunContext | None:
        context_path = path / "run.json"
        if not context_path.is_file():
            return None
        return RunContext.model_validate_json(context_path.read_bytes())
    ...
    def _load_from_path(self, run_id: str, path: Path, published: bool) -> StoredRun | None:
        ...
        cutoff_path = path / "frozen-evidence.json"
        if cutoff_path.is_file():
            cutoff_payload = json.loads(cutoff_path.read_bytes())
    ...
    def publish_atomically(self, bundle: PublishedRunBundle) -> PublishedArtifact:
        ...
        if bundle.markdown_sha256 is not None and bundle.markdown_sha256 != markdown_sha256:
    ...
    def get_latest(self, market_date: date) -> str | None:
        latest_path = self._safe_path(
            "reports", f"{market_date.year:04d}", market_date.isoformat(), "latest.json"
        )
        if not latest_path.is_file():
            return None
        try:
            run_id = json.loads(latest_path.read_bytes())["run_id"]
    ...
    def get_report(self, run_id: str) -> str | None:
        ...
        market_date, _ = self._validated_run_id(run_id)
        index_path = self._safe_path(
            "reports", f"{market_date.year:04d}", market_date.isoformat(), "index.json"
        )
        if not final.is_dir() or not report_path.is_file() or not index_path.is_file():
            return None
        try:
            index_entry = json.loads(index_path.read_bytes())[run_id]
            report_bytes = report_path.read_bytes()
            bundle_bytes = (final / "bundle.json").read_bytes()
    ...
    def load_published_bundle(self, run_id: str) -> PublishedRunBundle | None:
        ...
        if not bundle_path.is_file():
            return None
        return PublishedRunBundle.model_validate_json(bundle_path.read_bytes())
